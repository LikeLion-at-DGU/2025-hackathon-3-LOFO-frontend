...
export default function Received() {
  ...
  const handleFeedback = async (outcomeId) => {
    try {
      const d = await getReceivedFormData(outcomeId);
      const submitted =
        !!d?.already_submitted ||
        !!d?.has_feedback ||
        !!d?.feedback_exists ||
        !!d?.submitted ||
        !!d?.defaults?.submitted;
      if (submitted) {
        alert("이미 후기를 작성한 작업물입니다.");
        return;
      }
    } catch (e) {
      console.warn("[후기쓰기] 사전 확인 실패, 화면으로 이동합니다.", e);
    }
    navigate(`/nopo/received/feedback?id=${outcomeId}`);
  };
