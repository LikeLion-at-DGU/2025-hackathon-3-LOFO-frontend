...
// // value → label (서버가 value만 내려줄 때 화면 표시용)
...
// const sanitize = (v) => {
//   if (!v) return v;
//   if (v.startsWith("SOMEWHAT_")) return v.replace("SOMEWHAT_", "");
//   if (v === "VERY_INSUFFICIENT") return "INSUFFICIENT";
//   return v;
// };
...
//     if (f.name) {
//       const p = `media/${String(f.name).replace(/^\/?media\/?/, "")}`;
//       return addBase(p);
//     }
...
//         // defaults: 서버가 {value,label} 또는 value 또는 label 로 줄 수 있음 → 라벨 state로 정규화
//         if (data?.defaults) {
...
//   // ...파일 상단의 SAT_VALUE/REF_VALUE/USE_VALUE 매핑은 그대로 사용
...
//     const payload = {
//       outcome_id: Number(outcomeId), // ← 필수
//       overall_satisfaction: sanitize(SAT_VALUE[satisfaction]),
//       reflection_level: sanitize(REF_VALUE[reflection]),
//       practical_use: sanitize(USE_VALUE[usability]),
...
//             } else if (active.name) {
//               const name = String(active.name).replace(/^\/?media\/?/, "");
//               next = addBase(`media/${name}`);
//             }
